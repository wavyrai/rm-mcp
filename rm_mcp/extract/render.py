...
def _render_rm_to_ocr_png(
    rm_file: Path,
    output_width: int = REMARKABLE_WIDTH,
    output_height: int = REMARKABLE_HEIGHT,
) -> Optional[Path]:
    """Render a .rm file to a temporary PNG suitable for OCR.

    Converts .rm -> SVG (via rmc) -> PNG (via cairosvg or inkscape),
    compositing onto a white background for optimal OCR contrast.

    Args:
        rm_file: Path to the .rm file
        output_width: Width of the output PNG
        output_height: Height of the output PNG

    Returns:
        Path to the temporary PNG file (caller must delete), or None if rendering failed.
    """
    import subprocess
